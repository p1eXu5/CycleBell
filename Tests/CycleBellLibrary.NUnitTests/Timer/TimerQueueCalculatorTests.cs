...
namespace CycleBellLibrary.NUnitTests.Timer
{
    ...
    public class TimerQueueCalculatorTests : IStartTimeTimePointName
    {
        ...
        private (Preset[] presets, TimeSpan[][] results) GetTestData()
        {
            ...
            {
                new TimePoint("Relative Test TimePoint 1", "1:00:00", TimePointType.Relative),
                new TimePoint("Absolute Test TimePoint", "23:59:00", TimePointType.Absolute),
                new TimePoint("Relative Test TimePoint 2", "1:00:00", TimePointType.Relative),
            });

            var loopNum = presets[2].TimePointCollection[0].LoopNumber;

            presets[2].TimerLoops[loopNum] = 2;
